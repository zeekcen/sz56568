...
class OrderAction extends BaseAction
{
	...
	public function ajax(){ 
		...
		if($do=='add'){
			...
			if(is_array($_POST['attr'])){
				foreach($_POST['attr'] as $res){
					$attr .= $res['title'].':'.$res['val'].'<br>';
					$field = $res['attrid'];
					$attrs[$res['attrid']] = $res['val'];
					if($r[$field]){
						$fields[$field]['setup'] =string2array($fields[$field]['setup']);			 
						if($fields[$field]['setup']['fieldtype']=='images'){							 
							$p_data=explode(':::',$r[$field]);					
							foreach($p_data as $k=>$r1){
								$p_data_arr=explode('|',$r1);
								$r['price'] = $p_data_arr[1]==$res['val'] ?  $p_data_arr[2] :$r['price'];
							}
						}else{
							$options=explode("\n",$r[$field]);
							foreach($options as $k=>$r1) {
								$v = explode("|",$r1);
								$r['price'] = $v[0]==$res['val'] ? $v[1] :$r['price'];
							}
						}
					}
				}
	...
	public function done(){
		...
		if( empty($this->Config['isuserbuy']) && empty($this->_userid))$this->error ( L('NOLOGIN'));
				...
				if($order['pay_code']=='Balance'){				
					if( $order['order_amount']>0 && $order['order_amount'] <= $user['amount']){
						...
						if($r){
							...
							//消费记录
							$spend['createtime'] = time();
							$spend['userid'] = $userid;
							$spend['username'] = $this->_username;
							$spend['type'] = 1;
							$spend['value'] = $order['order_amount'];
							$spend['order_sn'] = $order['sn'];
							$spend['note'] = L('PAY_ORDER_NOTE').'(SN:<a href="'.YP_PATH.'/index.php?g=User&m=Order&a=show&id='.$orderid.'">'.$order['sn'].'</a>)';
							$r = M('PaySpend')->add($spend);
						...
						$paybutton='<span><input type="button"  class="button" onclick="window.location.href =\''.URL("User-Pay/pay").'\'" value="'.L('Recharge').'" /></span>';
