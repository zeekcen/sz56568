...
*/
	...
	*/
	...
	public function leased_get_list() {
		...
		$services = M('Services')->where("start_name like '%{$start_name}%' and end_name like '%{$end_name}%'")->page("{$page}, {$page_size}")->select();
		if ( ! count($services)) {
			$result['status'] = 0;
	...
	public function leased_get_favorite_list() {
		...
		$user_id = $_REQUEST['user_id'];
		$page = $_REQUEST['user_id'];
		$page_size = $_REQUEST['page_size'];

		$services = M('Servicesfavo')->where("user_id={$user_id}")->page("{$page}, {$page_size}")->select();
		if ( ! count($services)) {
			$result['status'] = 0;
