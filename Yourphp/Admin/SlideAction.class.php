...
class SlideAction extends AdminbaseAction {
    ...
    {	
		parent::_initialize();
		$this->Tplpath = YOURPHP_PATH.'Template/'.$this->sysConfig['DEFAULT_THEME'].'/';
		$this->Xmlpath = YOURPHP_PATH.'Template/'.$this->sysConfig['DEFAULT_THEME'].'/xml/';
	...
	function edittpl(){
		...
		if($_POST['content']){
			...
			echo ' <form method="post" id="myform"  action="'.U('Slide/edittpl').'">Slide_'.$_GET['tpl'].'.html<input type="hidden" name="tpl" value="'.$_GET['tpl'].'"/><textarea  name="content" id="content" style="width:98%;height:400px;margin:5px;"  >'.$content.'</textarea>  <input type="hidden" name="isajax" value="1" />
