...
class LangAction extends AdminbaseAction {
    ...
    {	
		parent::_initialize();
		$this->langpath = YOURPHP_PATH.'Template/'. THEME_NAME.'/lang/'.LANG_NAME.'/'; 
    }
	...
	function insert() {
		...
		$lang_path =YOURPHP_PATH.'Template/'. THEME_NAME.'/lang/'.$_POST['mark'].'/';
		$r =dir_copy(YOURPHP_PATH.'Template/'. THEME_NAME.'/lang/cn/',$lang_path); 
	...
	function param()
	{
		...
		foreach($files as $key => $file) {
			...
			if($temp[0]=='common'){
				$lang_files[$key]['name'] = L('LANG_common');
			}else{
				$lang_files[$key]['name'] = L('LANG_module');
			}
		}
		$this->assign ( 'id', $id );
