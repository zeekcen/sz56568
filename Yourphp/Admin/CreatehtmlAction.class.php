...
class CreatehtmlAction extends AdminbaseAction {
    ...
    public function _initialize()
    {
        ...
        foreach ((array)$this->module as $rw){
			if(($rw['issystem']==1  && $rw['status']==1)) $data['module'][$rw['id']] = $rw;
        }
	...
	public function doCreatelist()
    {
			...
			if(!isset($catids[$doid])){
					...
					$id = $catids[$doid];
					if(empty($pages)){
						$this->create_list($id,1,$count);
						$listrows =  $listrows ? $listrows : C('CREATE_LISTROWS');
						if(empty($count)) $count = max(intval(C('CREATE_TOTAL')), 1);
	...
	public function doUpdateurl()
    {
		...
		if($catids[0] > 0){
				...
				if($catids[$doid]<=0){
					...
					$dao = $M[$module] ?  M('Content') : M($module);
					$where = "catid=$id";
					$p = max(intval($p), 1);
					$start = $pagesize*($p-1);
					if(!isset($count)){
				...
				if($moduleid){
					...
					if(!$M[$module])$this->error(L('DO_EMPTY'));
					$where['mid'] = array('eq',$moduleid);
				}
				$p = max(intval($p), 1);
				$start = $pagesize*($p-1);
				...
				$pages = ceil($count/$pagesize);
				if($count){
					$list =  M('Content')->field('id,catid,url')->where($where)->limit($start . ',' . $pagesize)->select();		 
					foreach($list as $r) {
						if($r['islink']) continue;
						$url = geturl($this->categorys[$r['catid']],$r,$this->Urlrule);
						unset($r['catid']);
	...
	public function updateurl()
    {
			...
			if($this->categorys){
				foreach ($this->categorys as $r){
					...
					if(!$this->module[$r['moduleid']]['issystem'])continue;
					$array[] = $r;
				}
				import ( '@.EXT.Tree' );	
				$str  = "<option value='\$id'  \$disabled>\$spacer \$catname</option>";
	...
	public function doCreateshow()
    {
			...
			if(!isset($catids[$doid])){
					...
					$module=$this->categorys[$id]['module'];
					$dao = M('Content');
					$where = "catid=$id and status=1";
					$p = max(intval($p), 1);
					$start = $pagesize*($p-1);

					if(!isset($count))	$count = $dao->where($where)->count();
					$pages = ceil($count/$pagesize);
		...
		public function docreatesitemap()
		{
			if($_GET['htmlmap']){
				...
				$run =unlink(YOURPHP_PATH.'sitemap.html');
			}
			...
			if($_GET['xmlmap']){
				...
				$xmldata = $Cxml->Cxml($array,YOURPHP_PATH.'sitemap.xml');
				$d=file_exists(YOURPHP_PATH.'sitemap.xml');;
			}else{
				$dun =unlink(YOURPHP_PATH.'sitemap.xml');
			}

			if(($_GET['htmlmap'] && $r) || ($_GET['xmlmap']&& $d) || $run || $dun){
