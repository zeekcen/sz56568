 ...
 */
class Chinabank extends Yourphp {
	public $config = array()  ;
    ...
    public function __construct($config=array()) {
		...
		$this->config['notify_url'] =  return_url('chinabank',$record);
		$this->config['return_url'] =  return_url('chinabank',$record);
    }
	...
	public function respond()
    {
        ...
        if ($v_md5str==$md5string)
        {
            if ($v_pstatus == '20')
            {
				...
				return order_pay_status($v_oid,'2',$v_amount,$record);
            }
        }
        else
        {
