...
<table  class="search_table" width="100%">
	<tr>
		<td class="search">
			...
			<?php if($module_name=='Pictures') { ?>
                        <select name="company_id">
                        <option value="">请选择</option>
                        <?php
                        $Type_8 = M('Type')->field("typeid,name")->where("parentid = 8")->order('typeid desc')->select();
                        ?>
                        <?php $n=1;if(is_array($Type_8)) foreach($Type_8 AS $row) { ?>
                        <option value="<?php echo $row['typeid'];?>" <?php if($company_id ==$row['typeid']) { ?>selected="selected"<?php } ?>><?php echo $row['name'];?></option>
                        <?php $n++;}unset($n); ?>
                        </select>
                        <?php } ?>
			<?php if($fields['catid']) { ?>
		...
		<thead>
      	...
      	<tbody>
			...
			<td align="center"><?php echo $vo['id'];?></td>
      		<td align="center"><?php if($vo['catid']) { ?>[<font color="green"><?php echo $categorys[$vo['catid']]['catname'];?></font>]<?php } ?> 
			...
			<td align="center"><img src="<?php echo $vo['img_url'];?>" width="120" height="120"/></td>
