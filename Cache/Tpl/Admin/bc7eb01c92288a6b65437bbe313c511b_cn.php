...
<?php $n=1;if(is_array($member_config)) foreach($member_config AS $vo) { ?>

	<?php if($vo[varname] =='member_getpwdemaitpl' || $vo[varname]=='member_emailchecktpl') { ?>
	<tr>
		<th width="140"><?php echo $vo['info'];?>:</th>
		<td><textarea id="<?php echo $vo['varname'];?>" name="<?php echo $vo['varname'];?>" rows="5" cols="50"><?php echo htmlspecialchars(stripslashes($vo['value']));?></textarea></td>
	</tr>
	<?php } elseif ($vo['varname']=='credit_exchange') { ?>
	<tr>
		<th width="140"><?php echo $vo['info'];?>:</th>
		<td>1<?php echo L('YUAN');?> :<input type="text" name="credit_exchange" value="<?php echo $vo['value'];?>"><?php echo L('DIAN');?> </td>
	</tr>
	<?php } else { ?>
	<tr>
		<th width="140"><?php echo $vo['info'];?>:</th>
		<td><?php echo Form::radio(array('field'=>$vo[varname],'options'=>array('1'=>L('yes'),'0'=>L('no'))),$vo['value']);?></td>
	</tr>
	<?php } ?>
