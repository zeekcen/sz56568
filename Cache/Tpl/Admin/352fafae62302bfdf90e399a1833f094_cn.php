...
<form name="YpForm" action="<?php echo U('Module/listorder');?>" method="post">
        ...
        <thead>
      	...
      	<tbody>
			...
			<a href="<?php echo U('Module/edit',array(id=>$vo['id']));?>"><?php echo L('edit');?></a>		| 
					<?php if($vo[status]==1) { ?>
						<a href="<?php echo U('Module/status',array(id=>$vo['id'],status=>0));?>"><font color="green"><?php echo L('enable');?></font></a>
					<?php } else { ?>				
						<a href="<?php echo U('Module/status',array(id=>$vo['id'],status=>1));?>"><font color="red"><?php echo L('disable');?></font></a>	
					<?php } ?>
				 |  <a href="<?php echo U('Module/delete',array(id=>$vo['id']));?>"  onclick="return confirm('<?php echo L('module_real_uninstall');?>')"><?php echo L('delete');?></a>				
			</td>      		
      		</tr>
      		<?php $n++;}unset($n); ?>
      	</tbody>
