...
<?php } ?>
 ...
 <form id="YpForm" name="YpForm" action="<?php if($action_name=='edit') {  echo U('Posid/update'); } else {  echo U('Posid/insert'); } ?>" method="post">
      	<table cellpadding=0 cellspacing=0 class="table_form" width="100%">

      	<tr>
		<td width="120"><?php echo L('Posid_name');?></td>
		<td><input type="text" name="name" class="inputbox" value="<?php echo $vo['name'];?>" validate="required:true,cn_username:true, minlength:2, maxlength:20" /></td>
		</tr>

		</table>
		<div class="btn">
		<?php if($action_name=='edit') { ?><input type="hidden" id="id" name="id" value="<?php echo $vo['id'];?>" /><?php } ?>
		<INPUT TYPE="submit"  value="<?php echo L('dosubmit');?>" class="button" >
		<input TYPE="reset"  value="<?php echo L('cancel');?>" class="button">
		</div>
      </form>
</div>
<!--Yourphp企业建站系统 www.yourphp.cn-->