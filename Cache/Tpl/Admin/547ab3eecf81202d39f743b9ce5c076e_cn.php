...
<div class="titlec">
<p><?php echo L('tit_msg');?></p>
</div>

      
<p><?php echo L('current_version');?> <?php echo $var;?> &nbsp;&nbsp;<?php echo L('uptime'); echo UPDATETIME;?></p>
<?php if($patchlist) { ?>
<form name="YpForm" id="YpForm" action="<?php echo U($module_name.'/index?do=1');?>" method="post">
<div class="table-list">
    <table width="100%" cellspacing="0">
        <thead>
      		<tr>
			<th width="340" align="left"><?php echo L('upgrade_var_list');?></th>
			<th width="110" align="left"><?php echo L('uptime');?></th>
      		</tr>
      	</thead>
      	<tbody>
		<?php $n=1;if(is_array($patchlist)) foreach($patchlist AS $vo) { ?>
      		<tr>      		 
			<td align="left"><?php echo $vo['file'];?></td>
      		<td align="left"><?php echo $vo['filemtime'];?></td>      		 	
      		</tr>
      	<?php $n++;}unset($n); ?>
      	</tbody>
    </table>
    ...
    <div class="btn">
	<font color="red"><?php echo L('cover_html_file');?></font></label><input type="hidden" name="menuid" value="<?php echo $menuid;?>"><input name="cover" id="cover" type="checkbox" value=1>
	<input type="button" class="button"  name="" value="<?php echo L('start_upgrade');?>" onclick="$('#YpForm').submit();">
	</div>
	</div>
</div>
</form>
<?php } else { ?>
<div class="titlec"><font color="red"><?php echo L('latest_version');?></font></div>
<?php } ?>