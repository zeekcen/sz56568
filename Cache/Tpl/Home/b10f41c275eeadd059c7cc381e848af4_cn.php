...
<?php } ?>
		...
		<div class="list">
		<ul class="txt">
			<?php $catsql = getcatsql($catid,'');C('CREATE_LISTROWS',15);if(C('CREATE_TOTAL')){$total=C('CREATE_TOTAL');}else{$total=M("Content")->where(" status=1  AND lang=1 $catsql[sql]   ")->count();C('CREATE_TOTAL',$total);}import ( "@.EXT.Page" );$page = new Page ( $total, 15 );if($catsql['urlrule'])$page->urlrule =$catsql['urlrule'];$pages = $page->show();unset($data);$data=M("Content")->field("*")->where(" status=1  AND lang=1 $catsql[sql]   ")->order("id desc")->limit($page->firstRow . ',' . $page->listRows)->select();unset($catsql);;?>
				<?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
