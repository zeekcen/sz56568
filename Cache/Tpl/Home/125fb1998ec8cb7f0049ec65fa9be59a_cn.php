...
jQuery.YourphpInc(['/Statics/Js/yourphp.admin.min.js','/Statics/Css/Wdate.css']);
</script>

<div class="left">
		<div class="pos"><b></b><span><?php echo L(catpos);?><a href="<?php echo URL();?>"><?php echo L(home_font);?></a> &gt <YP:catpos  catid="catid" space=" > " /></span></div>
...
#content {margin:0;line-height:22px;}
		...
		<?php C('CREATE_LISTROWS',10);if(C('CREATE_TOTAL')){$total=C('CREATE_TOTAL');}else{$total=M("Guestbook")->where(" status=1  AND lang=1 $catsql[sql]   ")->count();C('CREATE_TOTAL',$total);}import ( "@.EXT.Page" );$page = new Page ( $total, 10 );$page->urlrule = '/index.php?m=Guestbook&a=index&id=19&l=cn|/index.php?m=Guestbook&a=index&id=19&l=cn&p={$p}';$pages = $page->show();unset($data);$data=M("Guestbook")->field("*")->where(" status=1  AND lang=1 $catsql[sql]   ")->order(" id DESC ")->limit($page->firstRow . ',' . $page->listRows)->select();;?>
			<?php $n=1;if(is_array($data)) foreach($data AS $vo) { ?>
			<dl>
				<dt><span><?php echo toDate($vo.createtime,'Y-m-d H:i:s');?></span><?php echo L(name);?>:<?php echo $vo['username'];?> - <?php echo $vo['title'];?></dt>
				<dd class="g_content">
				<?php echo $vo['content'];?>
				</dd>
				<?php if($vo['reply_content']) { ?><dt><?php echo L(admin_reply);?>:</dt><dd class="reply_content"><?php echo $vo['reply_content'];?></dd><?php } ?>
			</dl>
			<?php $n++;}unset($n); ?>
		...
		<div class="post_box">
		<h5><?php echo L(submit_book);?></h5>
		<form name="YpForm" id="YpForm" action="<?php echo URL('User-Post/insert');?>" method="post">
		...
		<?php $n=1;if(is_array($fields)) foreach($fields AS $r) { ?>
			<?php if(!empty($r['status']) && $r['ispost']) { ?>
				<tr>
					<td width="100" align="right"><?php if($r['required']) { ?><font color="red">*</font><?php } ?> <?php if(APP_LANG) {  echo L($r[field]); } else {  echo $r['name']; } ?></td>
					<td id="box_<?php echo $r['field'];?>">
					<?php echo getform($r);?>
					</td>
				</tr>
			<?php } ?>
		...
		<tr>
				<td width="100"></td>
				<td><INPUT TYPE="submit"  value="<?php echo L(dosubmit);?>" class="button" >
		<input TYPE="reset"  value="<?php echo L(cancel);?>" class="button"></td>
				</tr>
		</table>
		</form>
		</div>
