...
<div class="left" style="border:none;">
	...
	<div class="icontact"><img src="Template/Default/images/cpbox.jpg" alt="" align="left" height="160" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Yourphp网站管理系统,是一款完全开源免费的PHP+MYSQL系统.核心采用了Thinkphp框架等众多开源软件,同时核心功能也作为开源软件发布。 集众多开源项目于一身的特点,使本系统从安全,效率,易用及可扩展性上更加突出.程序内置SEO优化机制，使企业网站更容易被推广.拥有企业网站常用的模块功能（企业简介模块、新闻模块、产品模块、下载模块、图片模块、招聘模块、在线留言、友情链接、会员与权限管理）。</div>
	...
	<ul class="pic">
		<?php $catsql = getcatsql($T[m_product_catid],'');unset($data);$data=M("Content")->field("*")->where(" status=1  AND lang=1 $catsql[sql]   ")->order("id desc")->limit(0,15)->select();unset($catsql);;?>
			<?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
	...
	<div class="ilist"><ul class="txt">
		<?php $catsql = getcatsql($T[m_news_catid],'');unset($data);$data=M("Content")->field("*")->where(" status=1  AND lang=1 $catsql[sql]   ")->order("id desc")->limit(0,15)->select();unset($catsql);;?>
			<?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
