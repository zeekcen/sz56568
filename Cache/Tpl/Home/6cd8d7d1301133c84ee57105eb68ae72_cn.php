...
<div class="left">
		...
		<div class="down_list">
		<?php $catsql = getcatsql($catid,'a.');C('CREATE_LISTROWS',1);if(C('CREATE_TOTAL')){$total=C('CREATE_TOTAL');}else{$total=M("Download")->where(" a.id=b.id AND  a.status=1  AND a.lang=1 $catsql[sql]   ")->count();C('CREATE_TOTAL',$total);}import ( "@.EXT.Page" );$page = new Page ( $total, 1 );if($catsql['urlrule'])$page->urlrule =$catsql['urlrule'];$pages = $page->show();unset($data);$data=M("Download")->field("*")->where(" a.id=b.id AND  a.status=1  AND a.lang=1 $catsql[sql]   ")->order(" a.id DESC ")->limit($page->firstRow . ',' . $page->listRows)->select();unset($catsql);;?>
			<?php $n=1;if(is_array($data)) foreach($data AS $vo) { ?>
			<dl class="img">
			<dt><a href="<?php echo $vo['url'];?>" target="_blank"><?php echo $vo['title'];?></a></dt>
			<dd class="desc"><?php echo L(Down_desc);?>:<?php echo $vo['description'];?></dd>
			<dd class="ext"><?php echo L(Down_ext);?>:<?php echo $vo['ext'];?> &nbsp;&nbsp;&nbsp;&nbsp;<?php echo L(Down_size);?>:<?php echo $vo['size'];?></dd>
			</dl>
			<?php $n++;}unset($n); ?>
