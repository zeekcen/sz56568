...
<?php } ?>
		...
		<?php $catsql = getcatsql($catid,'a.');C('CREATE_LISTROWS',15);if(C('CREATE_TOTAL')){$total=C('CREATE_TOTAL');}else{$total=M("Picture")->where(" a.id=b.id AND  a.status=1  AND a.lang=1 $catsql[sql]   ")->count();C('CREATE_TOTAL',$total);}import ( "@.EXT.Page" );$page = new Page ( $total, 15 );if($catsql['urlrule'])$page->urlrule =$catsql['urlrule'];$pages = $page->show();unset($data);$data=M("Picture")->field("*")->where(" a.id=b.id AND  a.status=1  AND a.lang=1 $catsql[sql]   ")->order(" a.id DESC ")->limit($page->firstRow . ',' . $page->listRows)->select();unset($catsql);;?>
			<?php $n=1;if(is_array($data)) foreach($data AS $vo) { ?>
			<li><span><a href="<?php echo $vo['url'];?>"><img src="<?php echo $vo['thumb'];?>"></a></span><a href="<?php echo $vo['url'];?>" target="_blank"><?php echo $vo['title'];?></a></li>
			<?php $n++;}unset($n); ?>
