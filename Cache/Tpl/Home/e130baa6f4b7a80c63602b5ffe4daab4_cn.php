...
<?php } ?>
		...
		<?php $catsql = getcatsql($catid,'a.');C('CREATE_LISTROWS',15);if(C('CREATE_TOTAL')){$total=C('CREATE_TOTAL');}else{$total=M("Product")->where(" a.id=b.id AND  a.status=1  AND a.lang=1 $catsql[sql]   ")->count();C('CREATE_TOTAL',$total);}import ( "@.EXT.Page" );$page = new Page ( $total, 15 );if($catsql['urlrule'])$page->urlrule =$catsql['urlrule'];$pages = $page->show();unset($data);$data=M("Product")->field("*")->where(" a.id=b.id AND  a.status=1  AND a.lang=1 $catsql[sql]   ")->order(" a.id DESC ")->limit($page->firstRow . ',' . $page->listRows)->select();unset($catsql);;?>
			<?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
				<dl class="img">
				<dd class="pic"><span><a href="<?php echo $r['url'];?>" target="_blank"><img src="<?php echo thumb($r['thumb']);?>" align="absmiddle" ></a></span></dd>
				<dt><a href="<?php echo $r['url'];?>" target="_blank"><?php echo $r['title'];?></a></dt>
				<dd><?php echo L(type);?>：<?php echo $Categorys[$r['catid']]['catname'];?></dd><dd><?php echo L(Specification);?>：<?php echo $r['xinghao'];?></dd><dd><?php echo L(price);?>：<b><?php echo $r['price']; echo L(RMB);?></b></dd>
				</dl>
			<?php $n++;}unset($n); ?>
