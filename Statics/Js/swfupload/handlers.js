...
{ 
	...
	if(ext == 1) {
		...
		var img = '<a href="javascript:void(0);" onclick="javascript:add_file(this,'+id+')" id="on_'+id+'" class="on"><div class="icon"></div><img src="./Statics/images/other_ext.png" width="80" alt="'+name+'" imgid="'+id+'" path="'+src+'"/></a>';
	}
	$('#thumbnails').append('<li><div id="attachment_'+id+'" class="img"></div></li>');
	$('#attachment_'+id).html(img);	
	var datas='<div id="uplist_'+id+'"><input type="hidden" name="status" id="status" value="0"><input type="text"  id="aids" name="aids[]"  value="'+id+'"  /><input type="text"  id="filedata" name="filedata[]" value="'+src+'"  /> <input type="text" id="namedata" name="namedata[]" value="'+name+'"  /> &nbsp;<a href="javascript:remove_this(\'uplist_'+id+'\');">移除</a> </div>';
...
function add_file(obj,id,status){
	...
	if($(obj).hasClass('on')){
		...
		if(num < file_limit){
			$(obj).addClass("on");
			var datas='<div id="uplist_'+id+'"><input type="hidden" name="status" id="status" value="'+status+'"><input type="text"  id="aids" name="aids[]"  value="'+id+'"  /><input type="text"  id="filedata" name="filedata[]" value="'+src+'"  /> <input type="text" id="namedata" name="namedata[]" value="'+name+'"  /> &nbsp;<a href="javascript:remove_this(\'uplist_'+id+'\');">移除</a> </div>';
			$('#myuploadform').append(datas);		
			ids.push(id);
		}else{
		alert('已经达到附件限制数');
